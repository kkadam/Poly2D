...
!       print*,"Running sm"
       ...
       !  set up smz for case of no assumed symmetry
          do jl = rlwb, rupb
             ...
             do kl = zlwb, zupb
                do kg = 2, numz-1
                   xm = 0.5*rhfinv(jl)*rBinv*( (zhf_g(kg) - zhf(kl))*(zhf_g(kg) - zhf(kl)) +rB * rB +rhf(jl) * rhf(jl) )
       ...
       !  set up smz for case of equatorial symmetry
          do jl = rlwb, rupb
             ...
             do kl = zlwb, zupb
                do kg = 2, numz-1
                   xp = 0.5*rhfinv(jl)*rBinv*( (zhf_g(kg)+zhf(kl))*(zhf_g(kg)+zhf(kl)) + rhf(jl)*rhf(jl) + rB*rB )
                   xm = 0.5*rhfinv(jl)*rBinv*( (zhf_g(kg)-zhf(kl))*(zhf_g(kg)-zhf(kl)) + rhf(jl)*rhf(jl) +rB*rB )
